...
$this->Cell(10,6,utf8_decode("COMUNIDADES"),0,1,'L');
...
//==========================================================================      Cuerpo del programas
   ...
   if(empty($_GET['num'])){
    ...
    $alumn=$conexion->query("SELECT * FROM comunidad order by Comunidad");
   }
   else
   {
    $alumn=$conexion->query("SELECT * FROM comunidad where Comunidad like '%".$_GET['num']."%'");
...
//    $alumn=$conexion->query("SELECT * from medica where idreceta = '$id'");
    ...
    for($i=0;$i<1;$i++)
            {
                $pdf->Row(array(utf8_decode('id'),utf8_decode('Comunidad')),'L');
            }
   ...
   $pdf->SetFillColor(224,235,255);
    ...
    $pdf->SetFont('Arial','',8);
        ...
        $pdf->Row(array(utf8_decode($alumno['id']),utf8_decode($alumno['Comunidad'])) ,'L');
        }
